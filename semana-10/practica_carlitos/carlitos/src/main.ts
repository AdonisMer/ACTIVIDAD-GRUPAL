...
mostrarMensaje("");
  ...
  mostrarMensaje("Cargando datos…");
  const respProducto = await cargarProductos();
  if (respProducto.estado !== "listo") {
    return mostrarMensaje(
      respProducto.estado === "error" ? "Error: " + respProducto.mensaje : "Cargando…"
  );
}
productos = respProducto.datos;
mostrarMensaje("");
